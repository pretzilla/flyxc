...
import { getTrackerFlags, LIVE_RETENTION_SEC, removeBeforeFromLiveTrack, Trackers } from 'flyxc/common/src/live-track';
...
export async function refresh(): Promise<TrackerUpdate> {
  ...
  const trackers = await getTrackersToUpdate(Trackers.Flyme, start, 200);
  ...
  trackers.forEach((tracker: TrackerForUpdate) => {
    const key = tracker[Datastore.KEY] as Key;
    const datastoreId = idFromKey(key);
    try {
      ...
      if (flymeId == null) {
        ...
        const startTimestamp = Math.max(start - LIVE_RETENTION_SEC * 1000, tracker.updated ?? 0);
        ...
        if (fix) {
          track = LiveTrack.create({
            ...
            alt: [fix[3]],
            timeSec: [fix[4]],
            flags: [getTrackerFlags({ valid: true, device: Trackers.Flyme })],
          });
