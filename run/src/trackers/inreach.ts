...
export async function refresh(): Promise<TrackerUpdate> {
  ...
  const trackers = await getTrackersToUpdate(Trackers.Inreach, start - INREACH_REFRESH_INTERVAL_SEC * 1000, 100);

  const result: TrackerUpdate = {
    deviceType: Trackers.Inreach,
  ...
  for (const tracker of trackers) {
    ...
    // Fetch an extra minute if some data were in flight.
    const lastFetch = tracker.updated ?? 0;
    const fetchFrom = Math.max(start - LIVE_RETENTION_SEC * 1000, lastFetch - 60 * 1000);
