...
export async function refresh(): Promise<TrackerUpdate> {
  ...
  const trackers = await getTrackersToUpdate(Trackers.Skylines, start - LIVE_MINIMAL_INTERVAL_SEC * 1000, 100);

  const result: TrackerUpdate = {
    deviceType: Trackers.Skylines,
  ...
  while (trackers.length > 0) {
    ...
    batchTrackers.forEach((tracker) => {
      ...
      const key = tracker[Datastore.KEY];
      sklIdToDsId.set(sklId, idFromKey(key));
